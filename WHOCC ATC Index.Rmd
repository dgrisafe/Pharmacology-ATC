...
df_atc_2therapeutic <- bind_rows(lapply(df_atc_1anatomical$links, whocc_atc_2therapeutic))
```

## 3. Pharmacological Subgroup

```{r}
df_atc_3pharmacological <- bind_rows(lapply(paste0(df_atc_2therapeutic$links,"/"), whocc_atc_2therapeutic))
```

## 4. Chemical Subgroup

```{r}
df_atc_4chemicalsubgroup <- bind_rows(lapply(df_atc_3pharmacological$links, whocc_atc_2therapeutic))
```
...
whocc_atc_5chemicalsubstance <- function(url_5chemicalsubstance = "https://www.whocc.no/atc_ddd_index/?code=D10AA&showdescription=no"){
  ...
  atc_row <- tibble(codes = NA, description = NA, link = url_5chemicalsubstance)
  ...
  if(class(xml_atc_5chemicalsubstance_ul)=="xml_node"){
    ...
    atc_row <- bind_cols(
      codes = atc_codes[2:length(atc_codes)],
...
# # works great
# whocc_atc_5chemicalsubstance(df_atc_4chemicalsubgroup$links[[9]])
# 
# # throws an error because there are no drugs in this category
# whocc_atc_5chemicalsubstance(df_atc_4chemicalsubgroup$links[[10]])
# 
# # throws an error because there are blank spaces between rows
# whocc_atc_5chemicalsubstance(df_atc_4chemicalsubgroup$links[[13]])
# 
# # throws an error because ATC code is blank, but there are rows with DDD data
# whocc_atc_5chemicalsubstance(df_atc_4chemicalsubgroup$links[[431]])
```

```{r, include = FALSE}
# # good for debugging if some entries do not work
# df_atc_5chem <- list()
# df_atc_5chem$l01 <- lapply(df_atc_4chemicalsubgroup$links[  1:100], whocc_atc_5chemicalsubstance)
# df_atc_5chem$l02 <- lapply(df_atc_4chemicalsubgroup$links[101:200], whocc_atc_5chemicalsubstance)
# df_atc_5chem$l03 <- lapply(df_atc_4chemicalsubgroup$links[201:300], whocc_atc_5chemicalsubstance)
# df_atc_5chem$l04 <- lapply(df_atc_4chemicalsubgroup$links[301:400], whocc_atc_5chemicalsubstance)
# df_atc_5chem$l05 <- lapply(df_atc_4chemicalsubgroup$links[401:500], whocc_atc_5chemicalsubstance)
# df_atc_5chem$l06 <- lapply(df_atc_4chemicalsubgroup$links[501:600], whocc_atc_5chemicalsubstance)
# df_atc_5chem$l07 <- lapply(df_atc_4chemicalsubgroup$links[601:700], whocc_atc_5chemicalsubstance)
# df_atc_5chem$l08 <- lapply(df_atc_4chemicalsubgroup$links[701:800], whocc_atc_5chemicalsubstance)
# df_atc_5chem$l09 <- lapply(df_atc_4chemicalsubgroup$links[801:900], whocc_atc_5chemicalsubstance)
# df_atc_5chem$l10 <- lapply(df_atc_4chemicalsubgroup$links[901:922], whocc_atc_5chemicalsubstance)
```

```{r}
df_atc_5chemicalsubstance <- bind_rows(lapply(df_atc_4chemicalsubgroup$links, whocc_atc_5chemicalsubstance))
```
